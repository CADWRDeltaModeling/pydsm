...
@click.option('--input-file', type=click.Path(exists=True), required=True, help='Path to input CSV file')
@click.option('--path', type=click.STRING, required=True, help='HECDSS path to write the data to')
@click.option('--units', type=click.STRING, required=True, help='Units for the data')
@click.option('--period-type', type=click.STRING, required=False, default = 'PER-AVER', help='period type for the data')
def create_repeating(datafile, input_file, path, units, period_type = 'PER-AVER'):
    # Load the input CSV file
    df_template_year = pd.read_csv(input_file, parse_dates=['datetime'], index_col='datetime')
    # Create the repeating time series
    df_repeating = repeating_timeseries.create_repeating_timeseries(df_template_year, start_year=df_template_year.index.year[0], end_year=pd.Timestamp.now().year)
...
@click.argument('datafile', type=click.Path(exists=True))
@click.option('--cpart', required=True, help='HECDSS cpart filter to the datasets in the DSS file')
@click.option('--end-year', required=True, type=int, help='End year for the extended time series')
def extend_repeating(datafile, cpart, end_year):
    ...
    with dss.DSSFile(datafile, create_new=False) as dh:
        for df,u,t in matches:
            df_extended = repeating_timeseries.extend_repeating_timeseries(df, end_year=end_year)
