...
def get_slice_indices(tbl, stime, etime):
    dindex = pd.date_range(
        start=get_start_time(tbl), freq=get_time_interval(tbl), periods=tbl.shape[0]
    )
...
def mins_since_origin(dstr, origin_date="1899-12-31"):
    ...
    return delt.total_seconds() / 60.0
...
def slice_hydro(infile, outfile, stime, etime):
    ...
    with h5py.File(infile, "r") as hf:
        with h5py.File(outfile, "w-") as nhf:
            ...
            nhf["/hydro"].attrs["Start time string"] = bytes(
                str(pd.to_datetime(stime).strftime("%d%b%Y %H%M").upper()), "utf-8"
            )
