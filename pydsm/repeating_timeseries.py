...
def extend_repeating_timeseries(df_repeating, end_year):
    ...
    if end_year <= last_year:
        raise Exception('end_year must be greater than the last year in the repeating year dataframe')
    ...
    while not has_data_for_year(df_repeating, start_year) and start_year > earliest_year:
        start_year -= 1
    ...
    # if data frame index is period, then create period range
    if isinstance(df_extended.index, pd.PeriodIndex):
        df_extended.index = pd.period_range(start=df_last_year.index[0], end=f'{end_year}-12-31', freq=freq)
    else:
        df_extended.index = pd.date_range(start=df_last_year.index[0], end=f'{end_year}-12-31', freq=freq)
    return df_extended