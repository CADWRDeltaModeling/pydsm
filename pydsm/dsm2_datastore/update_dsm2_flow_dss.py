...
def update_dsm2_flow_dss(sdate, edate, dss_path, dss_outfile, station_id):

    process_flow_data = read_ts_repo(station_id = station_id, variable = 'flow', start= sdate, end = edate, repo = 'processed')
    if process_flow_data.empty:
       raise ValueError(f"No flow data found for station {station_id} for the period {sdate} to {edate}")
...
_DEFAULT_CONFIG_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "flow_config.yaml")
