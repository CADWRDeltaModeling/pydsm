...
def get_table_paths(filename, group_paths=['/hydro/input', '/hydro/data',
...
def read_table_as_df(h5, tpath):
    ...
    if x.dtype.names:
        result = pd.DataFrame(
            [[v[k].astype(str).strip() if v.dtype.fields[k][0].name.startswith(
                'bytes') else v[k] for k in v.dtype.fields] for v in x],
...
def convert_time_to_table_slice(start_time, end_time, interval, table_start_time, table_time_length):
    '''
