...
def linear_timeseries(request):
    ...
    ts = pd.DataFrame(
        np.arange(0, nvals),
        columns=["values"],
        index=pd.date_range(start="01JAN2000 0100", periods=nvals, freq="h"),
    )
...
def test_nse(linear_timeseries):
    assert 1 == tsmath.nash_sutcliffe(
        linear_timeseries.iloc[
            :,
            0,
        ],
        linear_timeseries.iloc[:, 0],
    )


def test_percent_bias(linear_timeseries):
    assert 0 == tsmath.percent_bias(
        linear_timeseries.iloc[:, 0], linear_timeseries.iloc[:, 0]
    )


def test_mean_error(linear_timeseries):
    assert 0 == tsmath.mean_error(
        linear_timeseries.iloc[:, 0], linear_timeseries.iloc[:, 0]
    )
